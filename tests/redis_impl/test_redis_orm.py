...
@pytest.mark.asyncio
async def test_save_calls_item_method(mocked_redis: MockedRedis) -> None:
    """
        Вызов метода сохранение одного элемента должен
            вызывать метод у самого элемента
    """
    mocked_item: MockedItem = MockedItem()
    await RedisORM(client=mocked_redis).save(item=mocked_item)
    assert mocked_item.calls_count == 1


@pytest.mark.asyncio
async def test_delete(mocked_redis: MockedRedis) -> None:
    """
        Проверка вызова метода delete для одного элемента
    """
    mocked_item: RedisItem = RedisItem()
    RedisItem._db_instance = None
    await RedisORM(client=mocked_redis).delete(item=mocked_item)
    assert mocked_redis.delete_calls_count == 1


@pytest.mark.asyncio
async def test_bulk_create_calls_methods(mocked_redis: MockedRedis) -> None:
    """
    ...
    RedisItem._db_instance = None
    await RedisORM(client=mocked_redis).bulk_create(items=items)
    assert mocked_redis._pipe.calls_count == items_count
    assert mocked_redis._pipe.execute_calls_count == 1


@pytest.mark.asyncio
async def test_bulk_delete_calls_methods(mocked_redis: MockedRedis) -> None:
    """
    ...
    RedisItem._db_instance = None
    await RedisORM(client=mocked_redis).bulk_delete(items=items)
