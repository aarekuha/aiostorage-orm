...
async def test_get_all(
    ...
    await test_frame.add(item_or_items=[test_item for _ in range(COUNT_OF_ITEMS)])
    result: list[RedisItem] = await test_frame.get(item=test_item)
    assert len(result) == COUNT_OF_ITEMS
    assert result[0] == test_item


@pytest.mark.asyncio
async def test_get_first_last(test_frame: RedisFrame) -> None:
    ...
    result: list[RedisItem] = await test_frame.get(item=item, start_index=0, end_index=0)
    assert len(result) == 1
